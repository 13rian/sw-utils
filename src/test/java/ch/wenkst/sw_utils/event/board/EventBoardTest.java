...
public class EventBoardTest {
	...
	private BoardEventSender eventSender;
	
	
	@BeforeAll
	public void setupExecutor() {
		eventSender = new BoardEventSender();
	}
	...
	public void allEventsAsynchronized() {
		...
		EventBoard eventBoard = new EventBoard(eventSender.getExecutor(), true);
		eventSender.setup(eventBoard);
		eventSender.fireEvents0to4();
	...
	public void sameEventSynchronized() {
		...
		EventBoard eventBoard = new EventBoard(eventSender.getExecutor(), false);
		eventSender.setup(eventBoard);
		eventSender.fireEvents0to4();
		eventSender.waitForEvent0to4();
		eventSender.unregisterLisener1();
