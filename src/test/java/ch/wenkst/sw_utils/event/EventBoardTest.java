...
public class EventBoardTest {
	...
	public void sameEventSync() {
		...
		// check if the timestamps are correct
		long[] timestamps1 = listener1.getReceivedTimestamps();
		Assertions.assertTrue(timestamps1[1] - timestamps1[0] >= 50, "listener1 timestamps"); 	// same event sync
		Assertions.assertTrue(timestamps1[3] - timestamps1[2] >= 50, "listener1 timestamps");  	// same event sync
		Assertions.assertTrue(timestamps1[2] - timestamps1[0] < 50, "listener1 timestamps"); 	// different event async
		Assertions.assertTrue(timestamps1[3] - timestamps1[1] < 50, "listener1 timestamps");  	// different event async
		
		
		// process time of listener 2 is 100, since all events are sent out async the difference needs to be below 100ms
		long[] timestamps2 = listener2.getReceivedTimestamps();
		Assertions.assertTrue(timestamps2[1] - timestamps2[0] >= 100, "listener2 timestamps"); 	// same event sync
		Assertions.assertTrue(timestamps2[3] - timestamps2[2] >= 50, "listener1 timestamps");  	// same event sync
		Assertions.assertTrue(timestamps2[2] - timestamps2[0] < 100, "listener2 timestamps"); 	// different event async
		Assertions.assertTrue(timestamps2[3] - timestamps2[1] < 100, "listener2 timestamps"); 	// different event async
	}
